...
- **waiter** → `WaiterApp` (bottom tabs: Τραπέζια / Δουλειές / Ιστορικό / Διαχείριση)
...
Το tab «Διαχείριση» (`ManageScreen`) φιλοξενεί τρεις content-only οθόνες με
εσωτερικά segments — Στατιστικά (`StatsScreen`), Κατάλογος (`MenuManagerScreen`),
Ρυθμίσεις (`SettingsScreen`) — πίσω από προαιρετικό 4ψήφιο PIN (`PinGate`,
`settings.adminPin`, ξεκλείδωμα in-memory ανά session). Το PIN είναι κλείδωμα
ευκολίας client-side, ΟΧΙ ασφάλεια — αυτή έρχεται με τα Firestore rules.

### State: τρία contexts, ένα ανά domain

| Context | Δεδομένα | Firestore |
|---|---|---|
| `AppContext` | τραπέζια, παραγγελίες, ιστορικό πωλήσεων, κατάλογος, ρυθμίσεις, ρόλος | `tables`, `sales`, `config/menu`, `config/settings` |
| `KitchenContext` | δελτία κουζίνας | `kitchenOrders` |
| `TasksContext` | δουλειές προς βοηθούς | `tasks` |

`config/settings` (`DEFAULT_SETTINGS` στο AppContext): στοιχεία επιχείρησης
(businessName/phone/address/vat/footerNote — θα μπουν στην εκτύπωση, βλ.
Μελλοντικά), `waitAlertMin` (ειδοποίηση αναμονής κουζίνας στα Τραπέζια),
`adminPin`. Το `saveSettings` ακολουθεί το pattern του `persistMenu`
(τοπικό state ΠΡΩΤΑ, μετά `setDoc`).
...
- Το `price` της γραμμής είναι Η ΤΕΛΙΚΗ τιμή μονάδας (βάση + άθροισμα των
  `delta` των options) — έτσι ΟΛΟΙ οι υπολογισμοί συνόλων/πληρωμής μένουν
  `price * qty` χωρίς να ξέρουν για options. Τα `options` υπάρχουν μόνο για
  εμφάνιση (σερβιτόρος, δελτίο κουζίνας, runner).
- `normalizeLine()` στο `AppContext` κανονικοποιεί παλιά δεδομένα (πριν το
  lineId/options) ώστε οι οθόνες να βασίζονται πάντα στο πλήρες σχήμα. Μην
  αφαιρεθεί όσο υπάρχουν παλιά docs στο Firestore.
- Η προσθήκη από τον κατάλογο συγχωνεύει ΜΟΝΟ σε γραμμή χωρίς `note` ΚΑΙ με
  ακριβώς τα ίδια options (`optionsKey`)· γραμμή με σημείωση ή διαφορετικά
  έξτρα μένει πάντα ξεχωριστή.
- Είδος καταλόγου: `{ id, name, price, options?: [{id, name, delta}] }` —
  τα έξτρα ορίζονται στη Διαχείριση → Κατάλογος → ✏️ και ο σερβιτόρος τα
  διαλέγει σε picker στο AddItems (είδη χωρίς options προστίθενται με ένα tap
  όπως πριν).
- Σημείωση ΟΛΗΣ της παραγγελίας: `table.orderNote` (`setTableOrderNote`) —
  μπαίνει από το TableDetail, ταξιδεύει στο επόμενο δελτίο κουζίνας
  (`kitchenOrders.orderNote`) και ΚΑΘΑΡΙΖΕΤΑΙ μετά την αποστολή (αφορούσε τη
  συγκεκριμένη παρτίδα).
- Μεταφορά παραγγελίας (`transferOrders(fromId, toId)`): όλες οι γραμμές
  μετακινούνται σε άλλο τραπέζι κρατώντας το `sentQty` (δεν ξαναμαγειρεύεται
  τίποτα). Δελτία που στάλθηκαν πριν τη μεταφορά δείχνουν το παλιό όνομα
  τραπεζιού στην κουζίνα — γνωστός περιορισμός.
...
Πληρωμή (`payItems`): επιλογή γραμμών στο bill sheet (split ανά είδος), μέθοδος
μετρητά/κάρτα, εγγραφή πώλησης στο `sales` με snapshot ειδών/συνόλου/ρέστων
(κάθε item snapshot κρατά και `category`/`options` για τα Στατιστικά), αφαίρεση
των πληρωμένων γραμμών από το τραπέζι.

### Ιστορικό & Στατιστικά

- `HistoryScreen`: αναζήτηση (τραπέζι/είδος/σερβιτόρος), ομαδοποίηση ανά ημέρα
  με «Σύνοψη ημέρας» (πληρωμές, 💵 μετρητά, 💳 κάρτα) — de facto Z αναφορά.
- `StatsScreen` (Διαχείριση → Στατιστικά): εύρη Σήμερα/7ημ/30ημ/Όλα, KPIs
  (έσοδα, πληρωμές, μέση απόδειξη, είδη/πληρωμή), μετρητά vs κάρτα, έσοδα ανά
  ώρα (μία σειρά, ένα hue, ετικέτα μόνο στην κορυφή), ανά κατηγορία, κορυφαία
  είδη. Δουλεύει πάνω στο ήδη φορτωμένο `history` — παλιές πωλήσεις χωρίς
  `category` στο snapshot πέφτουν στο «Άλλα».
...
- **Κεντρική παλέτα στο `src/theme.js`** (`import { C } from '../theme'`):
  ζεστοί «ανθρώπινοι» τόνοι — φόντο `C.bg` (σκούρο καφέ-γκρι), κάρτες `C.card`,
  inputs `C.field`, accent `C.accent` (καραμελέ — κύριες ενέργειες/ενεργά chips),
  `C.green` (χρήμα/σύνολα), `C.orange` (κουζίνα/προσοχή), `C.red`
  (καταστροφικό/καθυστέρηση), `C.blue` (runner/δουλειές). ΚΑΝΕΝΑ hardcoded hex
  σε οθόνες — κάθε νέο χρώμα μπαίνει στο theme.js με σημασιολογικό όνομα.
  Το `HIT_SLOP` εξάγεται κι αυτό από εκεί.
...
- Placeholders/hints: `C.placeholder` (ορατό σε ήλιο)· `C.faint` μόνο για
  καθαρά διακοσμητικά μεταδεδομένα.
...
- Δελτία κουζίνας: χρόνος αναμονής σε λεπτά, πορτοκαλί ≥10′, κόκκινο ≥20′
  (σταθερές `WARN_MIN`/`LATE_MIN` στο `KitchenScreen`). Στα Τραπέζια του
  σερβιτόρου, τραπέζι με pending δελτίο > `settings.waitAlertMin` λεπτά
  παίρνει κόκκινο περίγραμμα + badge «⏱ κουζίνα Χ′» (0 = off).
- Φίλτρα Τραπεζιών: Δικά μου / Όλα / Ελεύθερα / Με παραγγελία (single-select
  chips, `FILTERS` στο TablesScreen).
...
**ΕΚΤΥΠΩΣΗ (σχεδιασμένο — δεν έχει υλοποιηθεί, απόφαση 2026-07-08):**

Η υποδομή υπάρχει ήδη: τα στοιχεία επιχείρησης (επωνυμία/τηλέφωνο/διεύθυνση/
ΑΦΜ/υποσέλιδο) αποθηκεύονται στο `config/settings` από τις Ρυθμίσεις. Όταν
υλοποιηθεί:

1. Νέο module `src/utils/print.js` με δύο functions:
   - `buildReceiptHtml(data, settings)` → HTML string 80mm-friendly:
     κεφαλίδα (businessName, address, phone, ΑΦΜ), γραμμές ειδών με options
     (`+ Καραμέλα`) και σημειώσεις, σύνολο, μέθοδος πληρωμής, δοσμένα/ρέστα,
     ημερομηνία/ώρα/σερβιτόρος, `footerNote` στο τέλος («δεν αποτελεί
     φορολογική απόδειξη»).
   - `printHtml(html)` — web-only (`Platform.OS === 'web'`): κρυφό iframe ή
     `window.open` + `document.write` + `print()`. Σε native τα κουμπιά
     κρύβονται (μελλοντικά `expo-print`).
2. Σημεία σύνδεσης (τρία, ένα ανά τύπο εντύπου όπως στο POSPal):
   - **Προσωρινός λογαριασμός**: κουμπί στο bill sheet του TableDetail —
     τυπώνει τις ΕΠΙΛΕΓΜΕΝΕΣ γραμμές πριν την πληρωμή.
   - **Απόδειξη πελάτη**: μετά το `payItems` ή αναδρομικά από κάρτα πώλησης
     στο Ιστορικό (το sale snapshot έχει ήδη ό,τι χρειάζεται).
   - **Δελτίο κουζίνας**: κουμπί στο ticket του KitchenScreen (items + options
     + note + orderNote + τραπέζι + ώρα).
3. Θερμικός εκτυπωτής: browser print σε PC/tablet με driver του εκτυπωτή
   αρκεί για αρχή. Απευθείας ESC/POS από web ΔΕΝ γίνεται — αν χρειαστεί,
   λύσεις: RawBT (Android intent), τοπικό bridge (μικρός HTTP server σε
   Windows όπως κάνει το POSPal), ή Cloud Function → εκτυπωτής δικτύου.

**ΜΕΛΛΟΝΤΙΚΑ FEATURES (αξιολογήθηκαν από POSPal 2026-07-08, σκόπιμα ΕΚΤΟΣ προς το παρόν):**
- **Πακέτο/takeaway** (απόφαση χρήστη: όχι τώρα): παραγγελίες χωρίς τραπέζι.
  Προτεινόμενη υλοποίηση όταν χρειαστεί: «εικονικό τραπέζι» με `type:'takeaway'`
  και αυτόματο όνομα «Πακέτο #N», auto-διαγραφή μετά την πλήρη πληρωμή —
  μηδενική αλλαγή στο υπόλοιπο data model.
- **Online QR μενού πελατών** (απόφαση χρήστη: όχι): read-only route (π.χ.
  `?menu=1` στο web) που δείχνει τον κατάλογο + QR στις Ρυθμίσεις. Αν γίνει,
  τα είδη θέλουν προαιρετικά `desc` και tags αλλεργιογόνων.
- **Συγχώνευση τραπεζιών** POSPal-style (merge/unmerge με κύριο τραπέζι
  χρέωσης): υλοποιήθηκε το απλούστερο `transferOrders` (μεταφορά όλων των
  γραμμών). Πλήρες merge/unmerge θέλει tagging γραμμών (`mergedFrom`) και
  δεν αξίζει πριν το subcollection refactor.
- **Αρίθμηση αποδείξεων/δελτίων** (Παραγγελία #N): θέλει atomic counter
  (`runTransaction` σε `config/counters`) — αναβλήθηκε μαζί με τα υπόλοιπα
  transactions.
...
- `sales` append-only (rules: όχι update/delete), query με όριο ημέρας +
  pagination· η «Σύνοψη ημέρας» του Ιστορικού να αντικαταστήσει πλήρως το
  `clearHistory`.
...
## Ιστορικό αλλαγών (2026-07-08) — Features από σύγκριση με POSPal

Έγινε ανάλυση 22 screenshots του POSPal (pospal.gr — ελληνικό σύστημα
παραγγελιοληψίας, ίδιο πρόβλημα με local server αντί Firebase) και
ενσωματώθηκαν τα απαραίτητα features που έλειπαν, χωρίς αρχιτεκτονικές
αλλαγές:

- **Έξτρα επιλογές ειδών (+τιμή)** — ορισμός στη Διαχείριση, picker στο
  AddItems, εμφάνιση σε γραμμή/δελτίο/runner, merge γραμμών κατά options.
- **Σημείωση παραγγελίας** (order-level) που ταξιδεύει στο δελτίο κουζίνας.
- **Μεταφορά παραγγελίας σε άλλο τραπέζι** (`transferOrders`).
- **Φίλτρα τραπεζιών** (Ελεύθερα/Με παραγγελία) + **ειδοποίηση αναμονής
  κουζίνας** στα τραπέζια του σερβιτόρου (`settings.waitAlertMin`).
- **Θέσεις ανά τραπέζι** (προαιρετικό `seats`, badge 🪑 στην κάρτα) +
  **μαζική δημιουργία τραπεζιών** («Τραπέζι Ν» — η αρίθμηση συνεχίζει από το
  μεγαλύτερο υπάρχον για να μην βγουν διπλά ονόματα).
- **«Σερβιρίστηκαν όλα»** στο TableDetail: ο σερβιτόρος κλείνει μαζικά τα
  έτοιμα δελτία του τραπεζιού του (ίδιο αποτέλεσμα με το «Σερβιρίστηκε» του
  runner ανά δελτίο).
- **Διαχείριση tab** με PIN: **Στατιστικά** (KPIs, ανά ώρα/κατηγορία, κορυφαία
  είδη), αναβαθμισμένος **Κατάλογος** (επεξεργασία ονόματος/τιμής, κατηγορίες
  CRUD, αναδιάταξη ↑↓, έξτρα), **Ρυθμίσεις** (στοιχεία επιχείρησης, όριο
  αναμονής, PIN).
- **Ιστορικό**: αναζήτηση + Σύνοψη ημέρας (πληρωμές/μετρητά/κάρτα).
- **Νέο οπτικό θέμα** `src/theme.js` — ζεστή παλέτα αντί για μπλε/φωσφοριζέ.

Συνειδητά ΕΚΤΟΣ (βλ. «ΜΕΛΛΟΝΤΙΚΑ FEATURES»): εκτύπωση (σχεδιασμένη),
πακέτο/takeaway, QR μενού, merge/unmerge, αρίθμηση αποδείξεων, θερμικοί
εκτυπωτές/Windows bridge, άδειες/trial (δεν αφορούν).
