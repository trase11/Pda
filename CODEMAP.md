...
> Synced: f4c0be4 + POSPal features/νέο θέμα (ίδιο commit με αυτό το αρχείο) · 2026-07-08 · καλύπτει: App.js, index.js, src/ (όχι node_modules, dist, assets)
...
|------|----------------------|---------|
| App.js | Root routing ανά ρόλο· ΜΟΝΟ ο waiter έχει NavigationContainer (tabs: Τραπέζια/Δουλειές/Ιστορικό/Διαχείριση) | `Root()`, `WaiterApp` |
| index.js | Expo entry point | `registerRootComponent` |
| app.json | Expo config· `experiments.baseUrl: "/Pda"` δεμένο με όνομα repo (Pages) | |
| src/theme.js | ΚΕΝΤΡΙΚΗ παλέτα (ζεστοί τόνοι) + HIT_SLOP — κανένα hardcoded hex σε οθόνες | `C`, `HIT_SLOP` |
| src/firebase.js | Firebase init· `firebaseEnabled` = όλα τα πεδία συμπληρωμένα | `db`, `firebaseEnabled` |
| src/context/AppContext.js | Κεντρικό state: τραπέζια, κατάλογος, ιστορικό, ρυθμίσεις, ρόλος, βάρδια, PIN· cloud/local dual mode | `writeTableOrders`, `markOrdersSent`, `payItems`, `persistMenu`, `saveSettings`, `unlockAdmin`, `addItemToTable(options)`, `setTableOrderNote`, `transferOrders`, `updateMenuItem`, `addCategory`/`deleteCategory`/`moveCategory`/`moveMenuItem`, `normalizeLine`, `optionsKey`, `DEFAULT_SETTINGS`, `SHIFT_MAX_MS` |
| src/context/KitchenContext.js | Δελτία `kitchenOrders` (τώρα με `orderNote` + `options` ανά item)· derived `pendingOrders`/`readyOrders` | `sendToKitchen(tableName, items, tableId, orderNote)`, `markReady`, `markServed` |
...
| src/screens/HistoryScreen.js | Ιστορικό: αναζήτηση, ανά ημέρα με Σύνοψη ημέρας (πληρωμές/μετρητά/κάρτα), καθαρισμός | `sections` useMemo, `filtered` |
| src/screens/ManageScreen.js | Tab «Διαχείριση»: header + segments (Στατιστικά/Κατάλογος/Ρυθμίσεις) μέσα σε PinGate | `TABS` |
| src/screens/StatsScreen.js | Content-only στατιστικά: εύρη ημερών, KPIs, μετρητά/κάρτα, ανά ώρα/κατηγορία, κορυφαία είδη | `RANGES`, `stats` useMemo |
| src/screens/SettingsScreen.js | Content-only ρυθμίσεις: στοιχεία επιχείρησης (για μελλοντική εκτύπωση), waitAlertMin, adminPin, αλλαγή ρόλου | `handleSave` |
| src/screens/MenuManagerScreen.js | Content-only κατάλογος: CRUD ειδών+κατηγοριών, αναδιάταξη ↑↓, editor έξτρα επιλογών | `openEdit`, `addOption`, μέσω `persistMenu` |
| src/data/menuData.js | Default κατάλογος: `[{id,name,icon,items:[{id,name,price,options?}]}]` — seed του `config/menu` | κατηγορία `'Φαγητά'` = κουζίνα |
...
| .github/workflows/deploy.yml | push σε `main` ή `claude/ios-shop-pda-app-o4lnu0` → `expo export` → GitHub Pages (`dist/`) | |

## Ροές (file:function hops)
1. **Παραγγελία → κουζίνα → σερβίρισμα**: AddItemsScreen:`handleAdd` (αν το
   είδος έχει options → picker → `addItemToTable(..., options)`, price =
   βάση+deltas) → `tables.orders` (sentQty=0) → TableDetailScreen:
   `handleSendToKitchen` (στέλνει ΜΟΝΟ το δέλτα `qty−sentQty`, μαζί options
   names + `table.orderNote`, μετά καθαρίζει το orderNote) →
   KitchenContext:`sendToKitchen` (addDoc `kitchenOrders` status:'pending')
   **+ ταυτόχρονα** AppContext:`markOrdersSent` (sentQty=qty) → KitchenScreen
   (beep αν onDuty) → `markReady` (status:'ready', readyAt) → RunnerScreen
   (beep αν onDuty) → `markServed` (deleteDoc — το δελτίο πεθαίνει, το τραπέζι
   κρατά τις γραμμές).
...
## Πού ζει το state
- **AppContext**: `tables`, `menu`, `history`, `settings` (cloud: Firestore `tables`/`sales`/`config/menu`/`config/settings` · local: AsyncStorage)· `role`/`waiterName` ΠΑΝΤΑ ανά συσκευή σε AsyncStorage· `onDuty` και `adminUnlocked` ΜΟΝΟ in-memory (σκόπιμα).
...
- **`lineId` είναι το κλειδί γραμμής, ΟΧΙ `itemId`** — δύο γραμμές ίδιου προϊόντος με διαφορετική σημείωση/έξτρα συνυπάρχουν. Merge από κατάλογο ΜΟΝΟ σε γραμμή χωρίς `note` ΚΑΙ με ίδιο `optionsKey(options)`.
- **`line.price` = τελική τιμή μονάδας (βάση + Σ deltas των options)** — όλα τα σύνολα μένουν `price*qty`· τα `options` είναι μόνο για εμφάνιση. Μην υπολογίσεις ξανά deltas πουθενά αλλού.
- **`table.orderNote` καθαρίζεται μετά την αποστολή στην κουζίνα** — αφορά την τρέχουσα παρτίδα, όχι το τραπέζι για πάντα.
- **`normalizeLine()` μένει** όσο υπάρχουν παλιά docs χωρίς lineId/options στο Firestore.
...
- **`persistMenu`/`saveSettings` ενημερώνουν ΠΡΩΤΑ το τοπικό state** πριν το `setDoc` — αλλιώς stale-base data loss.
- **Το PIN (`settings.adminPin`) είναι κλείδωμα ευκολίας client-side** — ΟΧΙ ασφάλεια· μην χτιστεί τίποτα «ευαίσθητο» πάνω του.
...
- `settings.waitAlertMin` (Ρυθμίσεις) ↔ badge/κόκκινο περίγραμμα στο TablesScreen (`oldestPendingMins` — δουλεύει με `o.tableId`, τα δελτία ΠΡΕΠΕΙ να συνεχίσουν να γράφουν tableId).
- `payItems` snapshot (`category`, `options`) ↔ StatsScreen ομαδοποίηση — αν αλλάξει το σχήμα του sale item, άλλαξε και τα Στατιστικά.
- PinGate ↔ `settings.adminPin` + `adminUnlocked` (AppContext) — το gate τυλίγει ΟΛΟ το ManageScreen.
...
| Εργασία | Πού |
|---------|-----|
| Αλλαγή χρωμάτων/παλέτας | src/theme.js (ΜΟΝΟ εκεί — σημασιολογικά tokens) |
| Αλλαγή default καταλόγου/τιμών | src/data/menuData.js (seed) ή runtime μέσω Διαχείριση→Κατάλογος |
| Έξτρα επιλογές είδους (+τιμή) | Διαχείριση→Κατάλογος→✏️ (updateMenuItem `options`) |
| Στοιχεία επιχείρησης / PIN / όριο αναμονής | Διαχείριση→Ρυθμίσεις (config/settings, `saveSettings`) |
...
| Όριο badge αναμονής στα Τραπέζια | Ρυθμίσεις → `settings.waitAlertMin` (0 = off) |
| Διάρκεια auto-τέλους βάρδιας | src/context/AppContext.js:`SHIFT_MAX_MS` |
| Νέος ρόλος/πόστο | RolePickerScreen + App.js:`Root()` |
| Νέο segment στη Διαχείριση | src/screens/ManageScreen.js:`TABS` |
| Firebase credentials | src/firebase.js |
| Deploy/hosting | .github/workflows/deploy.yml + app.json:baseUrl |
| Μελλοντική εκτύπωση (σχέδιο) | CLAUDE.md «ΕΚΤΥΠΩΣΗ (σχεδιασμένο)» — module src/utils/print.js |
